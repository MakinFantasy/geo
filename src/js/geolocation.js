  ...
  setLocation() {
    if (navigator.geolocation) {
      ...
      this.chatInput.dataset.geoResponse = JSON.stringify({
        ...
        message: 'Смените браузер, либо введите местоположение вручную',
      });
    }
  }
  ...
  handleFailure(error) {
    ...
    switch (error.code) {
      case error.PERMISSION_DENIED:
        ...
        response.message = 'Измените настройки конфиденциальности, либо введите местоположение вручную';
        break;
      case error.POSITION_UNAVAILABLE:
        response.header = 'Информация о вашем местоположении недоступна';
        response.message = 'Введите местоположение вручную';
        break;
      case error.TIMEOUT:
        response.header = 'Истекло время ожидания запроса вашего местоположения';
        response.message = 'Нажмите "Отмена" и попробуйте ещё раз, либо введите местоположение вручную';
        break;
      default:
